...
class Maya(object):
	...
	def run(self):
		...
		starter_dir = os.environ['STARTER']
		...
		os.environ['PYTHONPATH'] = os.pathsep.join([scripts_dir, starter_dir])
		os.environ['MAYA_SCRIPT_PATH'] = os.pathsep.join([scripts_dir, starter_dir, qube_dir])

		# Iterate trough the list of user preferences directory
		for path in maya_user_prefs:
			# Check if directory is exist use it
			if os.path.exists(path):
				print "User preferences set to", path
				os.environ['MAYA_APP_DIR'] = path

		# Run application
		subprocess.Popen(maya_exec)
		print 'Runing command: ', maya_exec
